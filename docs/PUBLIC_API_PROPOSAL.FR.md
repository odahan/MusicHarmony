# Enaxos Music Theory — Contrat d'API publique .NET 8
...
### 5.2 Structures et relations de gammes
...
`ScaleStructures` retourne les distances cycliques d'une gamme sur une octave, par exemple `WWHWWWH` pour la gamme majeure. `ScaleRelations` calcule les notes communes en conservant l'ordre et l'orthographe de la première collection ; le mode de comparaison précise si l'on compare les graphies exactes ou les classes de hauteur.
...
`ScaleHarmony` construit une triade par degré en empilant les degrés alternés d'une gamme réalisée de trois à douze sons. Les triades standard sont reconnues comme définitions d'accord connues ; les autres reçoivent un identifiant stable dérivé du degré.
...
## 13. Ordre d'implémentation historique

Cet ordre a guidé la construction initiale de la bibliothèque et reste utile pour comprendre les dépendances entre modules :
...
Chaque étape est accompagnée de tests des invariants correspondants.
