...
Statut : spécification courante alignée sur l'implémentation
Cible : .NET 8.0
Nature : conception indépendante maintenue

## 1. Intention

Le moteur représente les concepts de théorie musicale et en déduit les résultats par des règles explicites. Il ne reproduit ni l'architecture, ni les algorithmes, ni les tables d'implémentation de l'ancien port de Mingus.

Les sources normatives de l'implémentation sont :
...
- les structures cycliques de gammes et les relations de notes communes ;
...
- les triades diatoniques dérivées d'une gamme réalisée ;
...
Une `ChordRealization` est une séquence ordonnée de notes octaviées réalisant un accord. Le renversement est déduit du degré d'accord placé à la basse. Un changement de réalisation ou de renversement ne modifie pas l'identité de l'accord.
...
- la basse reconnue lorsqu'elle est déterminable ;
- les hauteurs reconnues dans l'entrée ;
- les notes manquantes ou supplémentaires ;
- un score brut et une confiance normalisée.

Une correspondance exacte est classée avant une correspondance avec omission ou ajout. L'orthographe des notes contribue au classement mais l'équivalence enharmonique peut être activée par option. Les noms abrégés ou complets sont produits ensuite par le service de présentation.
...
La structure d'une gamme est calculée à partir des offsets chromatiques de sa formule ou des classes de hauteur d'une collection ordonnée. Une structure valide contient au moins deux sons, n'a pas de doublon de classe de hauteur et ferme exactement une octave.

### 5.5 Construction d'un accord

Chaque son théorique est obtenu depuis la fondamentale par le degré de formule. Les doublures, omissions et dispositions appartiennent à la réalisation octaviée, pas au `Chord`.

Une triade diatonique est construite depuis une gamme en prenant le degré courant, le degré deux positions plus loin et le degré quatre positions plus loin, avec retour cyclique dans la gamme.
...
- `INV-N05` — Un chiffre romain dérivé d'une `ScaleChord` n'est produit que pour une gamme heptatonique.
...
- tests des structures de gammes et des notes communes par classe de hauteur et par graphie exacte ;
- tests d'accords depuis plusieurs fondamentales avec altérations ;
- tests des triades diatoniques et de leur formatage en chiffres romains ;
